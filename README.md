...
| **Picture-in-picture** | A clone of the main camera, mirroring its position and orientation each frame, renders the same view into a 30%-sized bottom-right viewport — a scaled-down thumbnail that previews how the piece reads from a distance. Suppressed when `isMobile` or `navigator.webdriver` (§ headless capture). |
