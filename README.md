...
├── .clangd                     Checkout-local native compilation database for editor diagnostics
...
├── geometry.js                 Sphere-pixel position math (pixelToSpherical)
...
│   ├── generate-digest-migration.mjs  Regenerates the digest-to-filename migration table from the v1 fixtures
...
├── .githooks/                  Staged pre-commit checks, pre-push lint/typecheck/importmap/workflow-helper tests, master fast-forward guard
...
`wasm.cpp` compiles to `holosphere_wasm.js` + `.wasm` and exposes a single `HolosphereEngine` class. At most one instance may be live per module — its effect and arenas are shared module-global storage — so `delete()` the current engine before constructing another; the constructor traps otherwise. That is one of four JS-reachable preconditions the bridge does not answer with a rejection value. The other three guard payload decoding: `setShaderChain(entries)`, `setShaderChainParameters(entries)` and `restoreFullConfigSnapshot(snapshot)` read their argument's properties, and a getter or `Proxy` trap on the payload that runs JS during that read traps the module if it re-enters any decoder or `delete()`s the engine. Those payloads must be plain data. `HolosphereEngine.isLive()` reports only the singleton, so a bootstrap that can run twice tests it first rather than constructing into the trap.
...
The `tools/` directory also holds browser utilities shared with the root simulator,
including banners, clipboard, parameter labels, and pointer handling. These modules
have no dependency on a design-tool page.

Regenerate the tool utility stylesheet with `npm run generate:tailwind`; the pinned
Tailwind dependency scans tool HTML and JavaScript. Commit `tools/tailwind.css` with
changes that introduce utility classes.
