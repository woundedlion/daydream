...
test('insertion backfills presets, fields and staggered groups atomically', async () => {
  ...
  for (const preset of document.preset_bank.presets) {
    assert.equal(preset.values['warp.speed'], 0);
    assert.equal(preset.values['warp.frequency'], 1);
    assert.equal(preset.values['warp.edge-width'], 0.1);
    assert.equal(preset.values['warp.envelope'], 'flat');
  }
  const frequency = document.descriptor.parameters.find(
    (parameter) => parameter.id === 'warp.frequency');
  assert.equal(frequency.interpolation.kind, 'LOG_POSITIVE');
  const angle = document.descriptor.parameters.find(
    (parameter) => parameter.id === 'warp.field-angle');
...
test('an operator with a snap-curve field commits', async () => {
  ...
  const iterations = store.document().descriptor.parameters.find(
    (parameter) => parameter.id === 'sample.fractal-iterations');
...
test('insertion bounds accommodate binary32 catalog defaults', async () => {
  ...
  const parameter = document.descriptor.parameters.find(
    (entry) => entry.id === 'displace.strength');
...
test('a shortest-turn field backfills as a period-1 periodic parameter', async () => {
  ...
  const direction = store.document().descriptor.parameters.find(
    (parameter) => parameter.id === 'displace.direction');
...
test('auto labels take the operator stem, numbered only where it is taken', async () => {
  ...
  assert.deepEqual(labels(store).slice(3, 7), ['warp3', 'warp1', 'warp', 'warp2'],
    'the stem every shipped document spells, then the lowest free suffix');
  assertGreen(store);
});
...
test('auto labels reuse the label the replaced span releases', async () => {
  ...
  assert.equal(store.replaceSpan(WARP, 1, [{ operator: 'warp.wave-shear.v2' }]).ok, true);
  assert.equal(labels(store)[WARP], 'warp');
  assert.equal(store.replaceSpan(WARP, 1, [{ operator: 'warp.curl-flow.v2' }]).ok, true);
  assert.equal(labels(store)[WARP], 'warp',
    'the span under replacement does not hold its own label against it');
  assertGreen(store);
});
...
test('a crossing cannot be removed, only replaced', async () => {
  ...
  assert.deepEqual(labels(store),
    ['camera', 'lens', 'project', 'sample', 'colorize']);
  const document = store.document();
  for (const preset of document.preset_bank.presets) {
    assert.equal(preset.values['sample.complexity'], 0);
    assert.equal(preset.values['project.singularity-fade'], 1);
...
test('another control, a structural edit, an undo or a redo ends the run', async () => {
  ...
  assert.equal(store.undo(), true);
  assert.equal(presetValue(store, 'hex-twin-wave', 'sample.pattern-freq'), 3);
  assert.equal(labels(store).includes('warp'), true,
...
test('the store adopts the scratch document and edits it', async () => {
  ...
  assertGreen(store);
  assert.deepEqual(labels(store),
    ['rotate', 'lens', 'project', 'sample', 'colorize']);
});
