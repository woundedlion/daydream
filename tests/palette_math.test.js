...
/** Verifies the procedural palette-list row and coefficient ordering. */
test('proceduralPaletteCpp emits a valid C++ palette-list row', () => {
...
test('generativePaletteCpp serializes the complete V4 recipe', () => {
  ...
  for (const field of ['curve', 'center', 'range', 'custom']) {
    assert.ok(source.includes(`recipe.chroma.axis.${field} =`));
    assert.ok(!source.includes(`recipe.chroma.${field} =`));
  }
